...
library(knitr)
# library(sf)
# library(spData) couldn't find proper key Fed Abbv -> long-name
```
...
@fig-age-plt The female population is much younger than the male population. Generally, women were not permitted to join chess clubs until the ealy 1900's. Vera Menchik was the first woman to compete in top-level tournaments in the late 1920's. Judit Pulgar and Hou Yifan are generally acknowledged as the best female chess players of all-time. Judit Polgar's peak rating put her No. 8 in the world. See [Wikipedia(https://en.wikipedia.org/wiki/Women_in_chess)] for more info. Here's one of Polgar's most brilliant games: [link to Youtube](https://www.youtube.com/watch?v=PPXND2T4HCA).
...
tb_sex_ratio <- tb_ratings |>
  ...
  mutate(age_range = fct_rev(age_range))
...
There are `r  dplyr::n_distinct(tb_sex_ratio$fed)` Feds and `r  dplyr::n_distinct(tb_sex_ratio$age_range)` age ranges (total = `r nrow(tb_sex_ratio)` groups). Of those `r nrow(tb_sex_ratio)` fed-age-group pairings, `r sum(is.na(tb_sex_ratio$F))` and `r sum(is.na(tb_sex_ratio$M))` have no female or male players, respectively, making their sex ratios *NA*. I think it would make sense to impute *NA* player counts to 1.

```{r}
#| label: fig-sex-ratio-impute
#| fig-cap: Violin plot of sex ratios in each age group using imputed player counts values.
#| warning: false

tb_sex_ratio_impute <- tb_sex_ratio |>
  mutate(
    F_impute = replace_na(F, 1),
    M_impute = replace_na(M, 1),
    sex_ratio_impute = M_impute / F_impute)

tb_sex_ratio_impute |> 
  mutate(age_range = fct_rev(age_range)) |> 
  ggplot(aes(age_range, sex_ratio_impute, fill = age_range)) +
  geom_violin() +
  geom_point(aes(size = F_impute + M_impute), alpha = 0.6) +
  labs(x = "age", y = "sex ratio (M:F)") +
  scale_y_log10()
```
...
n_distinct(tb_sex_ratio_NA$fed)
```

```{r}
#| label: fig-sex-ratio-NA-hist
#| warning: false

tb_sex_ratio_NA_long <- tb_sex_ratio_NA |> 
  pivot_longer(c(F,M), names_to = "sex", values_to = "n_players") |> 
  mutate(age_range = fct_rev(age_range)) |> 
  filter(is.na(n_players))

tb_sex_ratio_NA_long |> 
  ggplot(aes(age_range, fill = sex)) +
  geom_histogram(stat = "count")

n_sex_ratio_NA <- sum(is.na(tb_sex_ratio$sex_ratio))
```

@fig-sex-ratio-NA-hist shows the number of Feds that have *NA* sex ratios for each age range and which sex is missing. Although some Feds have age groups with only female players, females are overwhelmingly more likely (`r round(sum(tb_sex_ratio_NA_long$sex == "F") / length(tb_sex_ratio_NA_long$sex) * 100, 0)`) to be absent than are males. Secondly, the number of missing males is relatively constant across ages groups, whereas the number of missing females increases.

```{r}
#| label: fig-sex-ratio-NA-ecdf

tb_males_imputed <- tb_sex_ratio_impute |>
  ungroup() |> 
  filter(is.na(M) | is.na(F)) |> 
  mutate(sex = "M", is_imputed = "imputed") |> 
  select(sex, is_imputed, n = M_impute)

tb_males_not_imputed <- tb_sex_ratio_impute |>
  ungroup() |> 
  filter(!(is.na(M) | is.na(F))) |> 
  mutate(sex = "M", is_imputed = "not imputed") |> 
  select(sex, is_imputed,  n = M)

tb_females_imputed <- tb_sex_ratio_impute |>
  ungroup() |> 
  filter(is.na(M) | is.na(F)) |> 
  mutate(sex = "F", is_imputed = "imputed") |> 
  select(sex, is_imputed, n = F_impute)

tb_females_not_imputed <- tb_sex_ratio_impute |>
  ungroup() |> 
  filter(!(is.na(M) | is.na(F))) |> 
  mutate(sex = "F", is_imputed = "not imputed") |> 
  select(sex, is_imputed, n = "F")

tb_n_sex_impute <- bind_rows(
  tb_males_imputed, tb_males_not_imputed,
  tb_females_imputed, tb_females_not_imputed
  ) |> 
  ungroup() |> 
  arrange(sex, n, desc(is_imputed)) |> 
  group_by(sex) |> 
  mutate(
    n_count = seq(length(n)),
    ecdf = n_count / length(n)) |> 
  mutate(color = c("imputed" = "coral2", "not imputed" = "black")[is_imputed])
  
# tb_n_sex_impute |> 
#   ggplot(aes(n, ecdf, color = is_imputed, shape = sex)) +
#   geom_point(
#     data = subset(tb_n_sex_impute |> filter(is_imputed == "not imputed")),
#     ) +
#   geom_point(
#     data = subset(tb_n_sex_impute |> filter(is_imputed == "imputed")),
#     size = 2) + 
#   scale_x_log10() +
#   facet_wrap(vars(sex))

tb_n_sex_impute |> 
  ggplot(aes(n, color = is_imputed, shape = sex)) +
  stat_ecdf(geom = "point", pad = F) +
  scale_x_log10() +
  facet_wrap(vars(sex))

Fn <- ecdf(tb_n_sex_impute |> filter(is_imputed == "imputed" & sex == "M") |> pull(n))

tb_males_impute_max <- tb_sex_ratio_impute |> ungroup() |> filter(is.na(F))  |> slice_max(M)
tb_males_ecdf_max <- tb_n_sex_impute |> filter(is_imputed == "imputed" & sex == "M") |>  slice_max(n)
```

@fig-sex-ratio-NA-ecdf is an empircal cumulative distribution function (ECDF) of players in each Fed-age-range pairing by sex and highlighting the imputation population. It shows that most of the *NA*s come from Fed-age-group pairings with low total populations (n < 10 for `r round(Fn(10) * 100, 0)`% of pairings). Although, there are some pairings that give very high sex ratios after imputation. The highest of these, for example, is `r tb_males_impute_max$fed[[1]]` `r tb_males_impute_max$age_range[[1]]` with `r tb_males_impute_max$M[[1]]` males (`r round(tb_males_ecdf_max$ecdf[[1]] * 100, 0)`th percentile in total male pop) and 0 females active that played a game in September. 
...
Would be cool to make a world map of total chess players and sex ratio by Fed over time. After looking for a while, can't find a dataset that maps Fed codes to their long name, making a join to a world map dataset more manual than I want to do. 
...
```{r}
#| label: best-youth
#| tbl-cap: Top 5 in age ranges under 20.

tb_top_youth <- tb_ratings |> 
  filter(month == "September") |> 
  filter(between(age, 0, 19)) |> 
  group_by(age_range, sex) |> 
  slice_max(rating, n = 5) |> 
  arrange(age_range, rating)

tb_top_youth |> kable()
```

```{r}
tb_top_youth_summarize <- tb_top_youth |> 
  group_by(sex, fed) |> 
  summarize(n_players = n(), avg_rating = mean(rating)) |> 
  arrange(desc(n_players), desc(avg_rating))

tb_top_youth_summarize |> kable()
```

India and Turkey the most top-5 strong young players. USA and CHN have particularly strong female players.