...
n_active <- n_distinct(tb_ratings_raw$id)
...
`r n_distinct(tb_removed$id)` members were removed, while `r n_distinct(tb_added$id)` members were added in September.
...
t.test(tb_ratings_change$rating_change)
```

There was a mean improvement in Elo of `r round(mean(tb_ratings_change$rating_change), 0)` .
