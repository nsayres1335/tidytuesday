...
Median Elo seems to peak around 25, but some of the data is too sparse in places. Need to use age groups. Also, looks like rating inflate.
...
The average player improved their Elo from August to September by `r round(mean(tb_ratings_change$rating_change), 0)` with 95% CI [`r t_test_change$conf.int[[1]]`, `r t_test_change$conf.int[[2]]`]. 
...
```{r}
#| label: title-stats
#| fig-cap: The number of women with titles shrunk generally. The number of male GMs increased.
...
There are `r  dplyr::n_distinct(tb_sex_ratio$fed)` Feds and `r  dplyr::n_distinct(tb_sex_ratio$age_range)` age ranges (total = `r nrow(tb_sex_ratio)`). Of those `r nrow(tb_sex_ratio)` fed-age-group pairings, `r sum(is.na(tb_sex_ratio$sex_ratio))` lack one or both sexes, giving, making the sex ratio for that pairing *NA*. 

```{r}
#| label: sex-ratio-explore

tb_sex_ratio |> group_by(age_range) |> slice_max(sex_ratio) |> kable()

tb_sex_ratio |> group_by(age_range) |> slice_min(sex_ratio) |> kable()
```
