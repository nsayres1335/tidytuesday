...
tb_sex_ratio_NA |> 
  mutate(age_range = fct_rev(age_range)) |> 
  ggplot(aes(age_range, fill = age_range)) +
  geom_histogram(stat = "count")

n_sex_ratio_NA <- sum(is.na(tb_sex_ratio$sex_ratio))
```

@fig-sex-ratio-NA-hist shows the number of Feds that have *NA* sex ratios for each age range. 
...
Fn <- ecdf(tb_males_not_imputed$n)
Fn(max(tb_males_imputed$n))
```

@fig-sex-ratio-NA-ecdf is an empircal cumulative distribution function (ECDF) of just the sex ratios that require imputation. It shows that most of the *NA*s come from Fed-age-group pairings with low total populations. Although, there are some pairings that give very high sex ratios after imputation. For example, ITA 50-59 has 266 males and 0 females active that played a game in September. 
